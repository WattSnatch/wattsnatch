...
### Air conditioning - MELCloud (`melcloud.js`)
- Polls Mitsubishi's MELCloud cloud API every 60 seconds (their hard rate limit) for every registered AC unit: power state, mode (Cool/Dry/Fan/Heat/Auto), set and room temperature, and daily/lifetime energy use.
- Feeds into baseline anomaly detection (below) so AC operation doesn't get mistaken for a fault.
