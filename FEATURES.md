...
**Optional: Bluetooth LE vehicle state (fully cloud-free).** Setting `tesla_state_source` to `ble` reads the car's state (battery, charging status, amps, charge limit) over the same BLE proxy instead of Fleet Telemetry, polling `vehicle_data` about every 5 seconds and using `body_controller_state` for a wake-free presence/sleep check. Because Bluetooth only reaches a few metres, reachability doubles as geofencing (in range = at home), so no GPS is needed - a car that goes briefly unreachable is not treated as having left until a run of checks fails, not just one, so a single dropped request cannot stop a charge that is still running. Set **both** `tesla_command_backend` and `tesla_state_source` to `ble` for a setup that needs no Fleet Telemetry, no Fleet API, and no Tesla OAuth token at all - everything happens over local Bluetooth. This cadence trades off against how often the car's own Bluetooth radio has to accept a fresh connection: Tesla vehicles cap how many devices (phone keys, watch keys, this proxy) can be connected at once, so a household with several keys in regular use may see this fall back to occasional retries rather than a clean read every cycle - the app absorbs that rather than treating it as a fault. Solar readings from the inverter are unaffected and still update every 5 seconds either way.
...
### Charging backend - Tesla or OCPP (`src/services/charging/`) *(OCPP built, not yet tested against real hardware)*
Everything above describes the Tesla path, which remains the default (`charging_backend` setting, defaults to `tesla`) and is unchanged - `controller.js` calls the same 9 functions and the same telemetry cache it always did, now routed through a dispatcher (`src/services/charging/index.js`) that forwards to the real `services/tesla.js` / `services/telemetry.js` with no logic in between, proven by `test/chargingBackendPassthrough.test.js` rather than merely claimed. Switching to `charging_backend: ocpp` (a restart-required setting, chosen during setup or in Settings) instead makes WattSnatch the **Central System** of an OCPP 1.6J connection: it listens on a WebSocket port (`ocpp_ws_port`, default 9220) and a charger connects in as the client, the standard OCPP topology. `setChargingAmps`/`startCharging`/`stopCharging` map to `SetChargingProfile`/`RemoteStartTransaction`/`RemoteStopTransaction`; `StatusNotification`/`MeterValues`/`StartTransaction`/`StopTransaction` from the charger populate the same telemetry-shaped state Tesla's Fleet Telemetry would. **No vehicle-reported battery percentage** on typical home AC chargers - reading real SoC over OCPP needs the vehicle to speak ISO 15118 ("Plug and Charge") to the charger, which almost no home wallbox supports, so charging runs at the solar-matched rate and the charge-limit stop condition can never fire for this backend (verified: `getChargeLimitAge()` is always `Infinity`) - the vehicle or charger ends the session on its own once satisfied.

**Departure scheduling is disabled on OCPP** (`departureScheduler.socAvailable()`) - "get me to 80% by 8am" cannot be answered without a starting percentage, and approximating it caused a real bug in development: the grid top-up latched on and never cleared, forcing hours of full-rate grid import before every departure, which is the exact opposite of this app's purpose. It now refuses at three layers - setting one, acting on one stored before a backend switch, and in the dashboard, which shows the reason and points at time-based scheduled windows instead. Automatic trip planning was already safe, bailing at an existing "battery SOC unknown" guard.

**Verified charging behaviour** (`test/ocppChargingLogic.test.js`) - the real control loop driving a real charge point, with only the solar meter stubbed: amps track solar up and down and clamp to `max_charge_amps`; a live EV draw reported by the charger is added back into the surplus rather than mistaken for house load (getting that wrong would stop charging dead); solar loss steps down to minimum and stops only after the hold timer, not instantly; CHARGE NOW and scheduled/free-power windows command full rate regardless of solar; the manual STOP button reaches the charger; and a charger dropping mid-charge doesn't disturb the loop. That last group needed a fix first: those paths were gated on holding a Tesla API token, which an OCPP install has no reason to have, so they were silent no-ops while ordinary solar charging (dispatched through an ungated path) worked fine - STOP appearing to work while not stopping the charger being the serious case.

**Protocol conformance** is verified twice over: against a real `ws` client running the full OCPP-J lifecycle (`test/ocppProtocol.test.js`), and independently against [`ocpp-rpc`](https://github.com/mikuso/ocpp-rpc) in strict mode (`test/ocppIndependentClient.test.js`), which validates every message against the *official* OCPP 1.6 JSON schemas - so a misreading of the spec shared by my own client and server would still be caught. Zero schema violations. Still **not tested against a physical charge point**, so treat as unverified against real hardware until it has been - see `OCPP-PLAN.md`.
...
- **SolarEdge** - cloud monitoring API (key + site ID). The legacy V1 API, deprecated on 3 November 2026.
- **SolarEdge (API V2)** - the replacement, on the SolarEdge ONE platform: OAuth 2.0 against a developer app you create yourself, so it needs no installer-issued key. Reads `/v2/sites/{id}/meters/telemetry`. Its finest resolution is a 15-minute average, and it is polled once per quarter-hour window to stay inside the free tier's 2,000 calls a month.
...
- **Sungrow** *(partially verified)* - **SH-series hybrid or SG-series string** inverter, local Modbus TCP via the WiNet-S dongle. The two product lines do **not** share a register map: SH hybrids expose energy-management data in a 13xxx block that SG string inverters do not have at all (reading it returns `Illegal data address`), and SG units carry their meter data in the 5xxx block instead. Selected by the **Inverter family** setting alongside the Sungrow connection details, defaulting to SH so existing installs are unchanged. Two things confirmed against real SG hardware: Sungrow documents register *numbers* which are 1-based while Modbus takes a 0-based *address*, so every address is the documented number minus one ("Total DC power" is documented at 5017 and reads at 5016); and the 13xxx block genuinely does not respond on an SG unit. The SG meter's **sign convention** is a setting rather than an assumption, since it is unverified across models and getting it backwards would invert every charging decision - verify it with `node scripts/diag-sungrow.js --host <ip> --watch` by switching a large load on and seeing which way the number moves. On SG units there is no battery to net out, so consumption is simply generation plus grid, and no lifetime-energy register is read (the controller's midnight-baseline fallback covers that) rather than guessing at an unverified address. Below, the SH-specific details:
- **Sungrow SH-series specifics** *(unverified against real hardware)* - local Modbus TCP via the WiNet-S dongle (same connection as the Sungrow battery integration - reuses its `sungrow_host`/`sungrow_port`/`sungrow_unit_id` settings, so a user with both configures the connection once). Register addresses and the 32-bit word-swapped decode convention are taken from evcc's published Sungrow template (see `THIRD_PARTY_LICENSES.md`), verified against a real Modbus TCP server built specifically to test this - including a scenario that exceeds 16 bits, since a wrong word-order bug would otherwise coincide with the right answer for any smaller residential system and go undetected. House consumption is derived (solar + grid import − battery charge) rather than a dedicated register, and nets out the battery's own charge/discharge power so a hybrid unit's attached battery isn't miscounted as house load. A hard plausibility bound (rejecting any reading no real SH-series system could produce) guards against a garbled or mis-decoded response being silently acted on. Still genuinely unverified against real hardware - please open an issue if yours reports something different.
...
**The target is held by WattSnatch, not pushed to the car.** Charging stops on the live battery reading the moment the target is met. This matters more than it sounds: Tesla refuses any charge limit below **50%**, and the minimum a short trip needs is routinely well under that (a 20 km round trip on a 20% floor lands near 31%), so a target expressed as a charge limit is silently rejected and the car keeps charging. Watching telemetry and issuing an explicit stop works at any percentage, and leaves the owner's own charge limit untouched. Covered by `test/tripChargeTarget.test.js`, which fails if either property regresses.
...
- Any trip marked `NEEDS_ATTENTION` gets a **Charge to X% for this trip** button on the dashboard, where X is the minimum that covers the round trip and keeps the 20% floor. It registers that target with the departure scheduler rather than commanding the car directly, so the same stop-on-telemetry rule applies and the trip is covered on the least grid power that will do it. If the trip is more than 6 hours away the button says it has scheduled the charge rather than claiming to have started one.
